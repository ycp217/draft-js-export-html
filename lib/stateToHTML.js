...
var MarkupGenerator = function () {
  ...
  function MarkupGenerator(contentState, options, injectCssOptions) {
    ...
    value: function writeStartTag(blockType) {
      ...
      try {
        for (var _iterator3 = tags[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          if (this.injectCssOptions[tag]) this.output.push('<' + tag + ' style="' + this.injectCssOptions[tag] + '">');else this.output.push('<' + tag + '>');
        }
      } catch (err) {
...
function stateToHTML(content, options, injectCssOptions) {
